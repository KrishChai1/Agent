...
class ResearchAgent(Agent):
    ...
    def _find_all_parts_generic(self, full_text: str, form_number: str) -> List[Dict]:
        ...
        for pattern in part_patterns:
            ...
            for match in matches:
                ...
                part_title = part_title[:100]  # Limit length
                
                # Convert to number if possible
                try:
                    part_num = int(part_id)
                except:
                    # For letters, convert to number (A=1, B=2, etc.)
                    if part_id.isalpha():
        ...
        # If no parts found, create a default
        if not parts:
    ...
    def _map_parts_to_pages(self, page_texts: List[str], all_parts: List[Dict]) -> Dict[int, List[Dict]]:
        """Map which parts appear on which pages"""
        part_page_map = {}
        ...
        for page_num, page_text in enumerate(page_texts):
            ...
            for part_info in all_parts:
                if self._is_part_on_page(part_info, page_text):
                    parts_on_page.append(part_info)
            
            if parts_on_page:
                part_page_map[page_num] = parts_on_page
        
        return part_page_map
    ...
    def _extract_fields_generic(self, page, page_num: int, text: str, part: str, 
                               part_number: int, part_title: str, form_number: str, 
                               parts_on_page: List[Dict]) -> List[ExtractedField]:
        """Generic field extraction for any USCIS form"""
        fields = []
        lines = text.split('\n')
        ...
        while i < len(lines):
            ...
            for pattern, pattern_type in patterns:
                ...
                if match:
                    ...
                    elif pattern_type in ['dash', 'letter']:
                        item_main = match.group(1)
                        item_sub = ""
    ...
    def _parse_item_number(self, item_num: str) -> Tuple[int, str, str]:
