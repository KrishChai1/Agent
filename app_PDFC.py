...
class FormExtractor:
    ...
    def _extract_fields_pattern(self, text: str, part_num: int) -> List[FormField]:
        ...
        patterns = [
            # Subfields with letters: 1.a. or 1a.
            (r'(\d+)\.([a-z])\.?\s+([^\n]{3,150})', 'subfield'),
            # Main fields: 1.
            (r'(\d+)\.\s+([^\n]{3,150})', 'main'),
            # Item Number format
            (r'Item\s+Number\s+(\d+)[.\s]*([^\n]{3,150})', 'item'),
            # Standalone letters (orphan subfields)
            (r'^([a-z])\.\s+([^\n]{3,150})', 'orphan')
        ]
        
        for pattern, ftype in patterns:
            flags = re.IGNORECASE | re.MULTILINE if ftype == 'orphan' else re.IGNORECASE
            matches = re.finditer(pattern, text[:15000], flags)
            ...
            for match in matches:
                try:
                    if ftype == 'subfield':
                        ...
                        # Find parent for orphan subfield
                    ...
                    # Check if this is a parent field (has subfields)
                    is_parent = self._is_parent_field(label, context, number, text)
        ...
        # Create missing parent fields
        self._create_parent_fields(fields, part_num)
