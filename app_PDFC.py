...
# ===== PART VERIFICATION AGENT =====

class PartExtractionDebugAgent:
    """Dedicated agent to debug and verify part extraction"""
    
    def __init__(self, client):
        self.client = client
    
    def debug_part_extraction(self, text: str, extracted_parts: List[Dict]) -> Dict:
        """Debug why parts might be missing"""
        debug_report = {
            "text_analysis": {},
            "pattern_analysis": {},
            "extracted_vs_expected": {},
            "missing_parts": [],
            "recommendations": []
        }
        
        # Analyze the raw text
        debug_report["text_analysis"] = self._analyze_raw_text(text)
        
        # Pattern-based analysis
        debug_report["pattern_analysis"] = self._pattern_based_analysis(text)
        
        # Compare extracted vs expected
        debug_report["extracted_vs_expected"] = self._compare_extraction(extracted_parts, debug_report["pattern_analysis"])
        
        # Generate recommendations
        debug_report["recommendations"] = self._generate_debug_recommendations(debug_report)
        
        return debug_report
    
    def _analyze_raw_text(self, text: str) -> Dict:
        """Analyze raw text for part indicators"""
        analysis = {
        ...
        # Find all mentions of "Part" followed by number
        ...
        for match in matches:
            ...
            analysis["part_mentions"].append({
                "part_number": part_num,
                "position": start_pos,
                "context": context.replace('\n', ' ').strip()
            })
            
            if part_num not in analysis["potential_parts"]:
                analysis["potential_parts"][part_num] = []
            analysis["potential_parts"][part_num].append(start_pos)
    ...
    def _pattern_based_analysis(self, text: str) -> Dict:
        ...
        for pattern in explicit_patterns:
            ...
            for match in matches:
                ...
                if part_num not in analysis["explicit_parts"]:
                    analysis["explicit_parts"][part_num] = {
                        "title": title,
                        "position": match.start(),
                        "pattern": pattern
                    }
        
        # Look for signature/contact sections that might be parts
        signature_patterns = [
            r'Contact\s+Information[,\s]*Certification[^\n]{0,100}',
            r'Interpreter[\'s\s]*Contact\s+Information[^\n]{0,100}',
            r'Preparer[\'s\s]*[^\n]{0,100}Contact[^\n]{0,100}',
            r'Additional\s+Information[^\n]{0,50}',
            r'Declaration\s+and\s+Signature[^\n]{0,100}'
        ]
        
        for pattern in signature_patterns:
            matches = re.finditer(pattern, text, re.IGNORECASE)
            for match in matches:
                analysis["signature_sections"].append({
                    "text": match.group(0),
                    "position": match.start(),
                    "pattern": pattern
                })
    ...
    def _compare_extraction(self, extracted_parts: List[Dict], pattern_analysis: Dict) -> Dict:
        """Compare what was extracted vs what patterns found"""
        comparison = {
            "extracted_count": len(extracted_parts),
            "extracted_numbers": [p["number"] for p in extracted_parts],
            "pattern_found_count": len(pattern_analysis["explicit_parts"]),
            "pattern_found_numbers": list(pattern_analysis["explicit_parts"].keys()),
            "missing_from_extraction": [],
            "extraction_details": []
        }
        
        # Find what patterns found but extraction missed
        pattern_numbers = set(pattern_analysis["explicit_parts"].keys())
        extracted_numbers = set(comparison["extracted_numbers"])
        
        comparison["missing_from_extraction"] = sorted(pattern_numbers - extracted_numbers)
        
        # Detail each extracted part
        for part in extracted_parts:
            comparison["extraction_details"].append({
                "number": part["number"],
                "title": part["title"],
                "found_in_patterns": part["number"] in pattern_numbers
            })
        
        return comparison
    
    def _generate_debug_recommendations(self, debug_report: Dict) -> List[str]:
        """Generate specific recommendations based on debug analysis"""
        recommendations = []
        
        text_analysis = debug_report["text_analysis"]
        pattern_analysis = debug_report["pattern_analysis"]
        comparison = debug_report["extracted_vs_expected"]
        
        # Check if parts are being found by patterns but not extracted
        if comparison["missing_from_extraction"]:
            recommendations.append(
                f"CRITICAL: Pattern analysis found {len(comparison['missing_from_extraction'])} parts "
                f"that extraction missed: {comparison['missing_from_extraction']}"
            )
        
        # Check text truncation
        if text_analysis["total_length"] < 10000:
            recommendations.append("WARNING: Text seems short - possible truncation issue")
        
        # Check if signature sections exist but aren't being detected as parts
        if len(pattern_analysis["signature_sections"]) > 2 and comparison["extracted_count"] < 6:
            recommendations.append(
                f"ISSUE: Found {len(pattern_analysis['signature_sections'])} signature sections "
                "but only extracted {comparison['extracted_count']} parts - signature sections not being detected as parts"
            )
        
        # Check for specific missing part numbers
        expected_parts = set(range(1, max(pattern_analysis["explicit_parts"].keys(), default=4) + 1))
        extracted_parts = set(comparison["extracted_numbers"])
        missing = expected_parts - extracted_parts
        
        if missing:
            recommendations.append(f"MISSING PART NUMBERS: {sorted(missing)}")
...
"""
    ...
    def _get_part_text_for_verification(self, full_text: str, part_number: int) -> str:
        """Extract part text for verification - enhanced for any number of parts"""
        # Multiple pattern attempts for robust part detection
        patterns = [
        ...
        if start_pos == -1:
            # If part not found, return a reasonable section of text
            section_size = len(full_text) // 8  # Assume 8 parts max
            start_estimate = (part_number - 1) * section_size
            return full_text[start_estimate:start_estimate + section_size + 5000]
        
        # Find next part boundary
        end_pos = len(full_text)
        for next_num in range(part_number + 1, part_number + 6):  # Check next few parts
            next_patterns = [
...
{text[:2000]}"""
    ...
    def extract_parts(self, text: str) -> List[Dict]:
        """Extract all parts/sections from any USCIS form - enhanced generic approach"""
        if not self.client:
            return self._fallback_part_extraction(text)
        
        prompt = f"""Analyze this COMPLETE USCIS form and identify ALL parts, sections, or major divisions.

CRITICAL: This form may have 3-10+ parts. You MUST find ALL of them including:

Look for ALL common USCIS form patterns:
- "Part 1. Information About You" / "Part 1. Applicant Information" / "Part 1. Petitioner Information"
- "Part 2. Application Type" / "Part 2. Petition Type" / "Part 2. Basis for Application"
- "Part 3. Processing Information" / "Part 3. Additional Information"
- "Part X. Contact Information, Certification, and Signature"
- "Part X. Interpreter's Contact Information"
- "Part X. Attorney or Representative Information"
- "Part X. Additional Information"

Look for ALL variations:
- "Part X." followed by title
- "Part X:" followed by title  
- Headers in different font sizes
- Signature/certification sections
- Contact information sections
- Attorney/interpreter sections

Scan EVERY page thoroughly and find all numbered parts.
...
{text}"""
        ...
        try:
            ...
            if "[" in content:
                ...
                # If AI found all parts, use it
                if parts and len(parts) >= 5:  # Should find at least 5+ parts
                    return parts
    ...
    def _fallback_part_extraction(self, text: str) -> List[Dict]:
        """Comprehensive fallback part extraction"""
        parts = {}
...
class UniversalFormProcessor:
    ...
    def _display_verification_results(self, verification_report: Dict):
        ...
        if overall_score >= 0.9:
            st.success(f"✅ Verification Complete: {overall_score:.1%} extraction accuracy")
        elif overall_score >= 0.75:
            st.warning(f"⚠️ Verification Complete: {overall_score:.1%} extraction accuracy - minor gaps detected")
        else:
            st.error(f"🔍 Verification Complete: {overall_score:.1%} extraction accuracy - significant gaps found")
        
        # Show part-by-part verification
        parts_verified = verification_report.get("parts_verified", {})
        if parts_verified:
            with st.expander("📊 Detailed Verification Report"):
                for part_num, part_data in parts_verified.items():
                    part_score = part_data.get("completeness_score", 0)
                    quality = part_data.get("extraction_quality", "unknown")
                    missing = part_data.get("missing_field_numbers", [])
                    
                    col1, col2, col3 = st.columns([2, 2, 3])
                    with col1:
                        st.metric(f"Part {part_num}", f"{part_score:.1%}")
                    with col2:
                    ...
                    if part_data.get("verification_notes"):
                        st.info(part_data["verification_notes"])
    ...
    def _attempt_field_recovery(self, form: USCISForm, full_text: str, verification_report: Dict):
        ...
        for part_num, part_data in verification_report.get("parts_verified", {}).items():
    ...
    def _extract_part_text(self, full_text: str, part_number: int) -> str:
        """Enhanced universal part text extraction for any number of parts"""
        # Try multiple part patterns
        part_patterns = [
            rf"Part\s+{part_number}\b",
            rf"PART\s+{part_number}\b", 
            rf"Part\s+{part_number}\.",
        ...
        if start_pos == -1:
            ...
            estimated_pos = (part_number - 1) * (len(full_text) // 8)  # Rough estimate
